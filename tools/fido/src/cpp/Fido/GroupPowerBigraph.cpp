...
Array<BasicBigraph> GroupPowerBigraph::iterativePartitionSubgraphs(BasicBigraph & bb, double newPeptideThreshold )
{
  ...
  ***/
  ...
  for (int k=0; k<preResult.size(); k++)
	...
	{
	  //	  cerr << LOG_MAX_ALLOWED_CONFIGURATIONS << " < " << logNumConfig << " and prunable: " << log2(bgb.PSMsToProteins.size()+1) << " " << bgb.proteinsToPSMs.size() << " [0]= " << bgb.originalN[0] << " --> (peps) " << bgb.PSMsToProteins.size() << endl;
