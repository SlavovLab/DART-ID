...
int main(int argc, char**argv)
{
  if ( argc == 2 )
      ...
      for (k=0; k<falsePositiveNames.size(); k++)
	{
      ...
      Array<double> empericalList, estimatedList, sensitivityList;
      ...
      while ( cin >> prob && getline(cin, line) )
	{
	  ...
	  //	  sensitivity = double(tpCount) / truePosSet.size();
	  sensitivity = double(tpCount);
	...
	cout << empericalList << " " << sensitivityList << endl;
    }
  else
    {
