...
class Model
{
 ...
 public:
  ...
  virtual double associatedEmission(int indexEpsilon) const
  {
    return alpha;
  }
  ...
  virtual double probabilityNoEmissionFrom(int indexEpsilon, int numActiveProts) const
  {
    //    return (1-spontaneousEmission())*pow( (1-associatedEmission()), numActiveProts);

    // using log for greater precision
    return pow(2.0, log2( 1-spontaneousEmission() )+numActiveProts * log2(1-associatedEmission(indexEpsilon)) );
  }
...
class DetectabilityModel: public Model
{
 protected:
 public:
  Array<double> alphaArray;
 ...
 DetectabilityModel():
  ...
  DetectabilityModel(const Array<double> & a, double b, double g)
    {
      alphaArray = a;
      alpha = -1;
      beta = b;
  ...
  double associatedEmission(int indexEpsilon) const
  {
    return alphaArray[indexEpsilon];
  }

  double probabilityNoEmissionFrom(int indexEpsilon, int numActiveProts) const
  {
    //    return (1-spontaneousEmission())*pow( (1-associatedEmission()), numActiveProts);
