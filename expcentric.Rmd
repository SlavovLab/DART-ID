...
ev.exp2 <- parse.ev.adj('dat/ev.adj.Fit2.expcentric.v2.txt');

# only use the same raw files between all evidence files
common.exps <- Reduce(intersect, list(ev.ali$Raw.file, ev.pep$`Raw file`, ev.exp$`Raw file`))
...
ev.exp2 <- ev.exp2[ev.exp$`Raw file` %in% common.exps,]
...
for(i in t) {
  ...
  id.frac[acc+counter] <- sum(ev.pep$PEP.new < i, na.rm=TRUE) / 
    sum(ev.pep$PEP < i & !is.na(ev.pep$PEP.new))
  id.frac[(2*acc)+counter] <- sum(ev.exp$PEP.new < i, na.rm=TRUE) / 
    sum(ev.exp$PEP < i & !is.na(ev.exp$PEP.new))
  id.frac[(3*acc)+counter] <- sum(ev.exp2$PEP.new < i, na.rm=TRUE) / 
    sum(ev.exp2$PEP < i & !is.na(ev.exp2$PEP.new))
  cat(counter, '/', acc, '                         \r')
  flush.console()
}
