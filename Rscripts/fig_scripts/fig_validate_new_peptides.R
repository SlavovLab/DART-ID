...
pdf(file='manuscript/Figs/rt_validation_v4.pdf', width=3.5, height=3.5)
...
error <- ev_c$`Retention time` - ev_c$rt_conf
hist(error[error > -0.6 & error < 0.6], breaks=seq(-0.6, 0.6, by=0.05),
     xlab=NA, ylab=NA, main=NA, xaxt='n', yaxt='n', ylim=c(0, 200))

axis(1, at=c(-0.6, -0.4, -0.2, 0, 0.2, 0.4, 0.6), tck=-0.02, mgp=c(0, 0.1, 0))
axis(2, at=seq(0, 150, by=50), tck=-0.02, mgp=c(0, 0.4, 0), las=1)

mtext('Residual RT (min)', side=1, line=1.1, cex=0.8)
mtext('Frequency      ', side=2, line=1.6, cex=0.8)
mtext('Set A vs. Set B', side=3, line=0.2, cex=0.8)

# plot histogram of error from intersected peptides that are confident
peps_a <- unique(ev_a %>% filter(!is.na(pep_new)) %>% filter(PEP < conf_thresh) %>% pull(`Modified sequence`))
peps_b <- unique(ev_b %>% filter(!is.na(pep_new)) %>% filter(PEP < conf_thresh) %>% pull(`Modified sequence`))
common_peps <- intersect(peps_a, peps_b)
rts_a <- ev_a %>% 
  filter(!is.na(pep_new)) %>% 
...
mtext('Residual RT (min)', side=1, line=1.1, cex=0.8)
mtext('Frequency      ', side=2, line=1.6, cex=0.8)
mtext('All confident peptides', side=3, line=0.2, cex=0.8)
