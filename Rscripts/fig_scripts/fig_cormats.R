...
barplot(c(mean(missing_a) / 6, mean(missing_b) / 6), width=1, space=0.5,
        ...
        xaxt='n', yaxt='n', xlab=NA, ylab=NA)
axis(1, at=c(-10, 1, 2.5, 10), labels=NA, tck=-0.02, mgp=c(0, 0.2, 0))
axis(2, at=seq(0, 1, by=0.025), labels=seq(0, 1, by=0.025)*100, 
...
plot(density(log10(ev_a$Intensity), na.rm=T))
lines(density(log10(ev_b$Intensity), na.rm=T), col='red')

plot(density(ev_a$PIF, na.rm=T))
lines(density(ev_b$PIF, na.rm=T), col='red')

mean(ev_a$`Missed cleavages`)
mean(ev_b$`Missed cleavages`)
