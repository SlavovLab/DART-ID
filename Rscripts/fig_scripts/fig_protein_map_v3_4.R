...
png(file='manuscript/Figs/protein_map_v4.png', width=3.5, height=6, units='in', res=250)
...
plot(0, 0, type='n', 
     ...
     xlab=NA, ylab=NA)
rasterImage(dmat_cc[nrow(dmat_cc):1,], 0, 0, ncol(dmat_c), nrow(dmat_c), interpolate=F)

# draw rectangle around new peptides

# bottom edge of the rect
ry0 <- sum(apply(dmat_c, 1, function(x) { any(x == 1)}))
ry1 <- nrow(dmat_c)
rect(xleft=0, xright=ncol(dmat_c), ybottom=ry0+20, ytop=ry1, 
     col=rgb(1,0,0,0.1), border=NA)
text(15, 6500, 'New Peptide IDs', adj=c(0, 0.5), cex=1, col='red')

axis(1, tck=-0.02, 
     at=seq(0, 270, by=30),
     labels=seq(0, 270, by=30),
     mgp=c(0, 0.1, 0))
axis(2, tck=-0.02, 
