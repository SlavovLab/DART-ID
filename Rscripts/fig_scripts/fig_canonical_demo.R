...
pdf(file='manuscript/Figs/canonical_demo_v2.pdf', width=3.5, height=2.5)

par(oma=c(0,0,1,0),
    mar=c(2,3,0.25,6.5), cex.axis=0.85)
...
exps <- c(0.06, 0.13, -0.14, -0.2, 0, 0.2)
...
points(rep(rep(seq(1,num_exps), 2), each=points_per_exp), rts, 
       pch=4,
...
legend(6.6, 20, c('Observed PSM', 'Missing PSM'), xjust=0, yjust=0.5, 
       pch=c(4, 1), pt.cex=1.25, lwd=2, lty=NA, text.width=0.3,
       col=c(rgb(0,0,0,1), rgb(0,0,0,0.3)),
       cex=0.85, x.intersp=0.4, y.intersp=1.2, bty='n', xpd=T)
...
mtext('Canonical RT', side=4, at=mus[1]+0.08, 
      line=0.3, las=1, col=cols[2])
mtext(bquote(.('Peptide ')*beta), side=4, at=mus[1]-0.08, 
      line=0.3, las=1, col=cols[2])

mtext('Canonical RT', side=4, at=mus[2]+0.08, 
