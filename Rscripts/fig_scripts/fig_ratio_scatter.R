...
ev_a <- ev.f1 %>% filter(PEP < 0.001)
#ev_b <- ev.f %>% filter(qval > 0.001 & qval_updated < 0.001) %>%
ev_b <- ev.f1 %>% filter(PEP > 0.001 & pep_updated < 0.001) %>%
...
png(file='manuscript/Figs/fig_ratio_scatter.png', width=5, height=5, units='in', res=250)

par(mar=c(0.05,0.1,0.05,0.1),
    oma=c(3, 3, 3, 3),
    mfrow=c(4,4), pty='s')
...
for(i in 1:4) {
  for(j in 1:4) {
    ...
    points(log2(ratio_mat_a)[,i], log2(ratio_mat_b)[,j],
         pch=16, cex=0.3, 
         #col=rgb(0,0,0,0.2),
         col=contour_cols[findInterval(dens, seq(0, 1.5, length.out=k))])
    
    abline(a=0, b=1, col='red', lwd=0.75)
    
    cor_text <- formatC(cor(log2(ratio_mat_a)[,i], log2(ratio_mat_b)[,j], method='pearson'), digits=3)
    text(-3.75, 3, bquote(.(as.name('rho'))*.('=')*.(cor_text)), adj=c(0, 0.5))
    
    # draw axes for marginal squares
    if(i == 4) {
      par(mgp=c(0, 0.2, 0))
      axis(1, at=c(-3, 0, 3), tck=-0.02)
    }
    if(j == 1) {
      par(mgp=c(0, 0.4, 0))
      axis(2, at=c(-3, 0, 3), tck=-0.02, las=1)
    }
    # label rows and columns on marginal squares
    if(i == 1) {
      mtext(parse(text=paste0('J[',floor((j-1) / 2)+1,']/U[',((j-1) %% 2)+1,']')), 
            side=3, cex=0.75, line=0)
    }
    if(j==4) {
      mtext(parse(text=paste0('J[',floor((i-1) / 2)+1,']/U[',((i-1) %% 2)+1,']')), 
            side=4, cex=0.75, line=0.3, las=1)
    }
  }
}

mtext('Log2 Jurkat / U937 Ratio − Spectra. PEP < 1e-3', side=1, outer=T, cex=1, las=1, line=1.75)
mtext('Log2 Jurkat / U937 Ratio − DART-ID. PEP < 1e-3', side=2, outer=T, cex=1, las=0, line=1.5)
mtext(paste0('Protein-Level RI Intensity, Distinct Peptides. n=', length(common_prots)), side=3, outer=T, cex=1, las=1, line=1.5)
