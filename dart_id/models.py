...
def generate_inits_two_piece_linear(dff, config):
    ...
    init_list['split_point'] = np.repeat(
        np.median(init_list['mu']), 
...
def muij_two_piece_linear(df, mu, params):
    ...
    muij[mu <= df['split_point']] = df['beta_0'] + (df['beta_1'] * mu)
...
models = {
    ...
    'two_piece_linear': {
        ...
        'exp_keys': ['beta_0', 'beta_1', 'beta_2', 'split_point', 'sigma_intercept', 'sigma_slope'],
        'pair_keys': ['muij', 'sigma_ij'],
        'peptide_keys': ['mu'],
