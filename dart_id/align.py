...
def get_os():
    ...
    if x == 'Linux': 
      ...
      if re.search(r'centos-7', p) is not None or re.search(r'rhel', p) is not None:
        ...
        raise Exception('Operating system {} not identified'.format(x))
...
def align(dfa, config):
  ...
  model_path = pkg_resources.resource_filename('dart_id', '/'.join((
      'models', model['model_binary'], exec_folder, exec_name
  )))
  ...
  peptide_params = pd.DataFrame({ 
      key: stan_results[key] for key in model['peptide_keys']
  })
  pair_params = pd.DataFrame({ 
      key: stan_results[key] for key in model['pair_keys']
  })
  ...
  exp_params = pd.concat([
          exp_params,
          pd.DataFrame({ 
              'init_'+key: init_list[key] for key in model['exp_keys']
          })
      ],
      axis=1
  )
  peptide_params = pd.concat([
          peptide_params,
          pd.DataFrame({ 
              'init_'+key: init_list[key] for key in model['peptide_keys']
          })
      ],
      axis=1
  )
  ...
  if config['save_params']:
      ...
      exp_params.to_csv(
          os.path.join(config['output'], 'exp_params.txt'), 
          sep='\t', index=False
      )
      pair_params.to_csv(
          os.path.join(config['output'], 'pair_params.txt'), 
          sep='\t', index=True, index_label='pair_id'
      )
      peptide_params.to_csv(
          os.path.join(config['output'], 'peptide_params.txt'), 
