...
class fido_network(bayesian_network):
    ...
    def cluster_proteins_from_lists(peptide_spectrum_proteins_prob_charge):
        ...
        for pep, spect, prots, prob, charge in peptide_spectrum_proteins_prob_charge:
            for prot in prots:
                if prot in prot_to_peps:
                    prot_to_peps[prot].update(set([pep]))
                else:
                    prot_to_peps[prot] = set([pep])

        peps_to_prots = {}

        for prot, pep_set in list(prot_to_peps.items()):
            pep_set = frozenset(pep_set)
            if pep_set in peps_to_prots:
                peps_to_prots[pep_set].update(set([prot]))
            else:
                peps_to_prots[pep_set] = set([prot])
...
def run_internal(df, parameter_map):
    ...
    if (
          parameter_map['gamma'] is None or 
          parameter_map['alpha'] is None or 
          parameter_map['beta'] is None
      ):

        logger.info('gamma, alpha, beta, not defined. choosing best parameters...')

        logger.info('estimating parameters with accuracy level {}'.format(parameter_map['parameter_accuracy']))
        ...
        logger.info('searching parameters for {} iterations'.format(len(gamma_search)*len(alpha_search)*len(beta_search)))
        ...
        for i, gamma in enumerate(gamma_search):
            for j, alpha in enumerate(alpha_search):
                for k, beta in enumerate(beta_search):
                    ...
                    posteriors = pd.DataFrame({'prot': ids, 'prob': probs}).sort_values('prob', ascending=False).reset_index(drop=True)
                    # group proteins by posterior probability
                    posteriors = posteriors.groupby('prob')['prot'].apply(lambda x: x.values).sort_index(ascending=False)
                    ...
                    logger.info('iteration {} || gamma: {}, alpha: {}, beta: {}'.format(iter_number, gamma, alpha, beta))
                    logger.info('                roc50: {:.4f}, fdr_mse: {:.4f}, objective: {:.4f}'.format(roc50, fdr_mse, current_objective))
    ...
    logger.info('running with parameters gamma: {}, alpha: {}, beta: {}'.format(parameter_map['gamma'], parameter_map['alpha'], parameter_map['beta']))
    ...
    out_df = pd.DataFrame({'Protein': ids, 'Probability': probs}).sort_values('Probability', ascending=False).reset_index(drop=True)
    ...
    out_df.to_csv(os.path.join(parameter_map['output'], 'protein_fdr.txt'), sep='\t', index=False)
...
def pd_main():
    ...
    parser.add_argument('-l', '--connected-protein-threshold', 
        type=int, default=14,
        help='log2 of maximum number of subgraph connected states'
    )
    parser.add_argument('-p', '--omit-clean-peptide-name', 
        action='store_true', default=False,
        help='omit cleaning the peptide names'
    )
    parser.add_argument('-a', '--all-psms', 
        action='store_true', default=False,
        help='use all PSM matches instead the best one'
    )
    parser.add_argument('-g', '--group-proteins', 
        action='store_true', default=False,
        help='use protein group level inference'
    )
    parser.add_argument('-s', '--prune-low-scores', 
        action='store_true', default=False,
        help='prune low scoring PSMs (threshold set at 0.01)'
    )
    parser.add_argument('-c', '--parameter-accuracy', 
        type=int, default=3,
    ...
    parser.add_argument('--proteins-column', 
        type=str, default='Proteins',
        help='column name for the proteins of each PSM'
    )
    parser.add_argument('--protein-delimiter', 
        type=str, default=';',
        help='delimiter character separating proteins in the proteins column'
    )
    parser.add_argument('--leading-protein-column', 
        type=str, default='Leading razor protein',
        help='column name for the leading (razor) protein of each PSM'
    )
    parser.add_argument('--decoy-tag', 
        type=str, default='REV__',
    ...
    parser.add_argument('--error-prob-column', 
        type=str, default='PEP',
        help='column name for the error probability of each PSM'
    )

    parser.add_argument('-o', '--output', 
        type=str, default=os.getcwd(), 
        help='Folder to output protein_fdr list and intermediate fido files'
    )
